...
```{r}
library(readr)
mental_health_expenditure <- read_csv("~/Muxue/academic/2019Fall/Stats122/data_science_project/mental_health_expenditure.csv")
```
...
```{r}
mental_exp <- mental_health_expenditure %>%
  mutate(State = tolower(`state`),exp = mental_health_expenditure)%>%
  select(-state)
  
state <-Suicide_Mortality_by_State %>% 
  mutate(state = tolower(abbr2state(`State`))) %>%
  left_join(mental_exp, by=c("state"="State"))
```
...
state%>% 
  ...
  theme_map(15)+
  labs(title = "Suicide Mortality by State Year 2017 (deaths per 100,000 people)")
...
state%>% 
  ...
  scale_fill_continuous(low="thistle2", high="darkred", guide="colorbar") +
  theme_map(15)+
  labs(title = "Suicide Mortality by State Year 2005 (deaths per 100,000 people)")
```
```{r}
state%>% 
  ggplot(aes(fill = RATE)) +
  ...
  labs(title = "Suicide Mortality by State Year (deaths per 100,000 people)")+
...
state%>% 
  ...
  scale_fill_continuous(low="thistle2", high="darkred", guide="colorbar") +
  theme_map(15)+
  labs(title = "Mental Health Expenditure 2013 (per capita)")
```
...
state %>%
  ...
  statebins_continuous(state_col = "State",
                     text_color = "white", value_col = "RATE",
                     brewer_pal="Reds", font_size = 3,
                     legend_title="Suicide Rate")
```
